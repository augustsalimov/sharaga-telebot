...
def _analize_sheet(sheet: Worksheet) -> List[Lesson]:
    ...
    for col in sheet.iter_cols(max_col=30, max_row=50):
        current_day: str = "00"
        for cell in col:
            ...
            if day:
                if current_day > day:
                    current_month = str(
                        int(current_month) + 1
                    )  # Если в колонке числится например март,
                    # но следом за 31 числом в той же колонке идет первое число
                    # то переводим месяц на один вперед
                    current_month = (
                        current_month if len(current_month) > 1 else "0" + current_month
                    )
                current_day = day
...
def _get_subject_info(cell: Cell, subject_column: str, sheet: Worksheet) -> dict:
    ...
    lesson_format_extra_info = (
        "" if not lesson_format_extra_info else " ({})".format(lesson_format_extra_info)
    )
